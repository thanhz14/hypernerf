...
def zero_adam_param_states(state, selector: str):
  ...
  for k in flat_params:
    if k.startswith(selector):
      ...
      flat_params[k] = adam._AdamParamState(
  ...
  new_state = OptimizerState(step, new_param_states)
  return new_state
...
def train_step(model: models.NerfModel,
  ...
  def _loss_fn(params):
    ret = model.apply({'params': params['model']},
                      batch,
                      extra_params=state.extra_params(),  # ← CHANGED: Call method
  ...
  # ===== THAY ĐỔI: Dùng params thay vì optimizer =====
  params = state.params
  
  # Handle disable_hyper_grads using gradient masking
  if disable_hyper_grads:
    # Create a mask function that zeros out hyper gradients
    def mask_hyper_grads(grads):
      # Zero out gradients for hyper_sheet_mlp parameters
      if 'model' in grads and 'hyper_sheet_mlp' in grads['model']:
        grads = dict(grads)
        grads['model'] = dict(grads['model'])
        grads['model']['hyper_sheet_mlp'] = jax.tree_map(
            lambda x: jnp.zeros_like(x), grads['model']['hyper_sheet_mlp'])
      return grads
  else:
    mask_hyper_grads = lambda x: x  # Identity function

  # ===== THAY ĐỔI: Compute gradients trên params =====
  grad_fn = jax.value_and_grad(_loss_fn, has_aux=True)
  (_, (stats, model_out)), grad = grad_fn(params)
  
  # Apply hyper gradient masking if needed
  grad = mask_hyper_grads(grad)
  ...
  # ===== THAY ĐỔI: Apply gradients với Optax API =====
